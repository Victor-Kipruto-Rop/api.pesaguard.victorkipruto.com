...
| # | Requirement | Status | Evidence | Gap |
|---|---|---|---|---|
| 7.1 | Backup | `PARTIALLY CERTIFIED` | `operations/backup_postgres.py` (compressed `pg_dump`, SHA-256 manifest, `PESAGUARD_BACKUP_ENCRYPT_COMMAND`, off-site upload, retention pruning, `last_status.json`), `tests/test_backup_integrity.py`, **15 passed** / **7 failed** in the Recovery chunk | 7 failures: 2× `FileNotFoundError (WinError 2)` invoking external binaries, **1× genuine test defect** (`UnboundLocalError: cannot access local variable 'compose'` at `test_phase8_backup_dr.py:269` — code placed after `pytest.skip`), and further path/tooling assumptions that only hold on Linux |
| 7.2 | Restore | `PARTIALLY CERTIFIED` | `operations/validate_restore.py` validates counts/constraints/indexes/audit/reconciliation/fraud storage and reports `passed`/`failed`; `operations/run_restore_drill.py`; `tests/test_disaster_recovery.py` **3 passed** | An actual restore into a real PostgreSQL instance was not executed here |
| 7.3 | DR | `NOT VERIFIED` | `docs/operations/DISASTER_RECOVERY.md` defines **7 failure drills** and an explicit exit gate; runbook at `docs/runbooks/DISASTER_RECOVERY.md` | **No drill artifact exists** — no restore-validation JSON, no `last_status.json`, no recorded start/end timestamps |
| 7.4 | RPO | `NOT VERIFIED` | Targets are documented | **Targets contradict each other**: `docs/operations/DISASTER_RECOVERY.md` states *near-zero* RPO; `docs/architecture/DISASTER_RECOVERY.md` and `docs/database/RESTORE_PROCEDURES.md` state *RPO ≤ 24 hours*. No measurement was ever recorded |
| 7.5 | RTO | `NOT VERIFIED` | Targets are documented | *RTO < 15 minutes* (operations) vs *RTO ≤ 4 hours* (architecture/database). No measurement was ever recorded |
