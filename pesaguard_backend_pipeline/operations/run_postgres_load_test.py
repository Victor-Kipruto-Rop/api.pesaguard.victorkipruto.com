...
# Support direct script invocation as well as package execution.
import sys
...
def main() -> None:
    ...
    result = run(required_env("DATABASE_URL"), max(1, count), max(1, args.concurrency), max(1, tenants), args.keep_table, args.inject_failures, args.stage)
