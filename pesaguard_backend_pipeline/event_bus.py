...
class EventEnvelope:
    ...
    def from_dict(cls, value: Dict[str, Any]) -> "EventEnvelope":
        try:
            fields = {"event_id", "event_type", "event_version", "occurred_at", "tenant_id", "aggregate_id", "payload", "correlation_id", "request_id", "attempt", "max_attempts"}
    ...
    def retry(self) -> "EventEnvelope":
        ...
        return EventEnvelope(**{**self.to_dict(), "attempt": self.attempt + 1})
...
class EventDeliveryController:
    ...
    def replay(self, dead_letter: DeadLetterEvent, handler: Callable[[EventEnvelope], Any], *, lag: int = 0) -> DeliveryResult:
        ...
        replay_event = EventEnvelope(**{**dead_letter.event.to_dict(), "attempt": 1})
        with self._lock:
